...
export class DietPlanPage implements OnInit {
  ...
  initData() {
    ...
    this.dateService.getEstiamateOfCalorieConsumption().then((res: any) => {
      ...
      this.apiCallService
        .getDietPlan(localStorage.getItem("c_id"))
        .subscribe((data: any) => {
  ...
  foodSelected(food, i, category) {
    ...
    if (result.length > 0) {
      let index = this.selectedFoodData.findIndex(
        (item) => item.c_id === result[0].c_id
      );
      this.selectedFoodData.splice(index, 1);
      switch (category) {
        case "morning_snack":
          this.morningCalories =
            this.morningCalories -
            parseInt(this.dietFoodMorningData[i].c_calories);
          this.dietFoodMorningData[i].isAddedInDiet = false;
          break;
        case "breakfast":
          this.breakfastCalories =
            this.breakfastCalories -
            parseInt(this.dietFoodBreakfastData[i].c_calories);
          this.dietFoodBreakfastData[i].isAddedInDiet = false;
          break;
        case "lunch":
          this.lunchCalories =
            this.lunchCalories - parseInt(this.dietFoodLunchData[i].c_calories);
          this.dietFoodLunchData[i].isAddedInDiet = false;
          break;
        case "evening_snack":
          this.eveningCalories =
            this.eveningCalories -
            parseInt(this.dietFoodEveningData[i].c_calories);
          this.dietFoodEveningData[i].isAddedInDiet = false;
          break;
        case "dinner":
          this.dinnerCalories =
            this.dinnerCalories -
            parseInt(this.dietFoodDinnerData[i].c_calories);
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: () => {
            ...
            this.apiCallService
              .storeFoodData(args, localStorage.getItem("c_id"))
              .subscribe(
                (response: any) => {
  ...
  scheduleAppointment(data) {
    this.apiCallService.scheduleAppointment(data).subscribe(
      (res) => {
        this.scheduled = true;
      },
      (err) => {
